...
const index = () => {
  let [type, setType] = useState("inbox");
  let [openSidebar, setOpenSidebar] = useState(false);
  let [openCommunityDetails, setOpenCommunityDetails] = useState(false);
  let [showUserDetails, setShowUserDetails] = useState(false);
  let [showCommunityModal, setShowCommunityModal] = useState(false);
  ...
  return (
    <div className="h-screen max-h-screen">
      ...
      {showCommunityModal && (
        <CreateCommunityModal setShowCommunityModal={setShowCommunityModal} />
      )}
      {/* Modal to add members to communities */}
      {showAddMembersModal && (
        <AddMembersModal setShowAddMembersModal={setShowAddMembersModal} />
      )}
    </div>
  );
};
